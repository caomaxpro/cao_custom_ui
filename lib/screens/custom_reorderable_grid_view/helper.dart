...
class CollisionResult {
  ...
  final Rect? collisionRect; // hoặc Offset nếu chỉ cần vị trí
  final String side; // "left", "right", "top", "bottom"

  CollisionResult({
    required this.object,
    required this.overlapRect,
    required this.collisionRect,
    this.side = "",
  });

  String getCollisionPositionString() {
    return '${object?.x}, ${object?.y}, ${collisionRect?.left}, ${collisionRect?.top}';
  }

  List<double> getCollisionPositionList() {
    return [?object?.x, ?object?.y, ?collisionRect?.left, ?collisionRect?.top];
  }
}
...
CollisionResult? getOverlappedObjectByDirection({
